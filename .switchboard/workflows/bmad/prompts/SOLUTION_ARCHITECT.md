...
**Filename format:** `solution-arch-{event}-{YYYY-MM-DD-HHMM}.md`

### When to Notify

| Event | Trigger | Message Content |
|-------|---------|-----------------|
| **Solutioning complete** | `.solutioning_done` created | Project name, epic count, story count, estimated total points |
| **Incremental complete** | FEATURES.md processed | New epic/story count, which areas affected |
| **Blocker found** | PRD is ambiguous or missing critical info | What's unclear, what decision was made |

### When NOT to Notify

- Phase completions (too noisy)
- Session resumptions
- Routine brownfield analysis
...
**Project:** switchboard-metrics
**Type:** Brownfield (existing codebase)

📊 Produced **4 epics, 18 stories** (42 points total)
- Epic 1: Core metrics collection (5 stories)
- Epic 2: Storage backend (4 stories)  
- Epic 3: API endpoints (6 stories)
- Epic 4: Dashboard integration (3 stories)

6 stories marked already-implemented. 12 new stories for dev agents.
```
